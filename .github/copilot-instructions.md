...
## Documentation & RST Standards (Repo Policy)

Apply these reStructuredText rules for all docs/templates/readme-like content created or edited in this repo. Keep changes docs-only; never change Python logic to satisfy docs style.

- Line width: target ≤ 100 chars; wrap paragraphs manually; keep URLs unbroken where practical.
- Headings: use consistent adornments and hierarchy; underline only (no overlines). Example levels:
    - H1: =, H2: -, H3: ~, H4: ^. Do not skip levels.
- Table of contents: prefer a local toctree per section when files grow; keep maxdepth small (2-3).
- Links:
    - Inline links for short references: ``Link text <https://example.com>``_.
    - Use named references for repeated links: define ``.. _odoo-guides: https://www.odoo.com/documentation`` and reference as ``odoo-guides_``.
    - Prefer relative links within this repo; keep anchors stable.
- Inline markup: use ````inline literals```` for code/field names; use ``*emphasis*`` and ``**strong**`` sparingly.
- Code blocks: use ``::`` or ``.. code-block:: <lang>`` with language hints (python, xml, csv, bash). Indent code 3+ spaces; avoid tabs.
- Admonitions: use ``.. note::``, ``.. warning::``, ``.. tip::`` for important calls; keep short and actionable.
- Lists and tables: prefer bullet and enumerated lists; for tables, use grid tables only when necessary; keep simple.
- Images: place assets under ``static/`` or ``docs/_static``; reference with ``.. image::`` and add ``:alt:`` text.
- Cross-references: when pointing to models/fields, prefer ``:code:`` literals; avoid Sphinx domain roles unless docs build is configured.
- Changelog/commit style: use conventional commits; mirror summaries in docs when relevant; avoid duplicating implementation details.
- i18n in docs: keep doc strings in English; UI strings in code must follow the Python i18n policy above.

Minimal doc skeleton when adding a new feature:
...
- API and model hooks (method names only, no internal stack traces)

Validation: run the "Validate Records Management Module" task; docs must not introduce syntax/build errors. If adding many rules, prefer a standalone ``docs/STYLE_GUIDE.rst`` and link it here.
