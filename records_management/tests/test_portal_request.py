"""
Test cases for the portal.request model in the records management module.
...
class TestPortalRequest(TransactionCase):
    """Test cases for portal.request model"""
    ...
    def _create_test_record(self, **kwargs):
        ...
        # Required fields for portal.request model:
        # - name (auto-generated)
        # - partner_id
        # - description
        # - company_id (default set)

        default_values = {
            'partner_id': self.partner.id,
            'description': '<p>Test portal request for automated testing</p>',
            'request_type': 'other',
            'priority': '1',
        }
        default_values.update(kwargs)

        return self.env['portal.request'].create(default_values)
    ...
    def test_update_portal_request_fields(self):
        """Test updating portal.request record fields"""
        # GitHub Copilot Pattern: Update operations
        record = self._create_test_record()

        # Test updating description
        new_description = '<p>Updated test description for portal request</p>'
        record.write({'description': new_description})

        self.assertEqual(record.description, new_description)


    def test_delete_portal_request_record(self):
        """Test deleting portal.request record"""
        ...
        self.assertFalse(self.env['portal.request'].browse(record_id).exists())
    def test_validation_portal_request_constraints(self):
        """Test validation constraints for portal.request"""
        # Test deadline cannot be before requested date
        ...
        with self.assertRaises(ValidationError):
            record.write({
                ...
                'deadline': past_date  # Deadline before requested date - should fail
            })

        # Test negative costs validation
        with self.assertRaises(ValidationError):
            record.write({'estimated_cost': -100.0})  # Negative cost - should fail

        with self.assertRaises(ValidationError):
            record.write({'actual_cost': -50.0})  # Negative cost - should fail
    ...
    def test_bulk_operations(self):
        ...
        # Test bulk create
        bulk_records = self.env['portal.request'].create(records)
