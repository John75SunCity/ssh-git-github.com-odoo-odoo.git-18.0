...
class CustomerInventory(models.Model):
    ...
    total_boxes = fields.Integer(
        ...
        store=True,
        help="Total number of boxes in this inventory",
    )
    total_documents = fields.Integer(
        string="Total Documents",
        compute="_compute_total_documents",
        store=True,
        help="Total number of documents in this inventory",
    )
    ...
    def _compute_total_boxes(self):
        """Compute total number of boxes from box_ids One2many relationship"""
        for record in self:
            record.total_boxes = len(record.box_ids)

    @api.depends("document_ids")
    def _compute_total_documents(self):
        """Compute total number of documents from document_ids One2many relationship"""
        for record in self:
