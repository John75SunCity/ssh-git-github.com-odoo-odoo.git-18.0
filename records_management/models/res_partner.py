...
class ResPartner(models.Model):
    ...
    total_records_containers = fields.Integer(
        ...
        help="Total number of records containers in storage",
    )
    ...
    total_monthly_revenue = fields.Float(
        string="Total Monthly Revenue",
        default=0.0,
        help="Total monthly revenue from this customer",
    )
    billing_account_number = fields.Char(
        string="Billing Account Number", help="Billing account number"
    )
    preferred_pickup_day = fields.Char(
        string="Preferred Pickup Day", help="Preferred day for pickups"
    )
    service_level = fields.Char(string="Service Level", help="Service level agreement")
    credit_limit = fields.Float(
        string="Credit Limit", default=0.0, help="Credit limit for this customer"
    )
    payment_terms_days = fields.Integer(
        string="Payment Terms Days", default=30, help="Payment terms in days"
    )
    ...
    def _compute_transitory_stats(self):
        ...
        for partner in self:
            if partner.is_company:
                ...
                partner.active_transitory_items = len(
                    transitory_items.filtered(
    ...
    def _compute_records_stats(self):
        """Compute records containers statistics"""
        for partner in self:
            if partner.is_company:
                records_containers = self.env["records.container"].search(
                    [("customer_id", "=", partner.id)]
                )
                partner.total_records_containers = len(records_containers)
            else:
