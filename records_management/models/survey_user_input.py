# -*- coding: utf-8 -*-
"""
Survey User Input Extension for Records Management
...
class SurveyUserInput(models.Model):
    """
    Extend survey.user.input to integrate with Records Management customer feedback system
    """
    _inherit = 'survey.user.input'
    ...
    def _compute_sentiment_analysis(self):
        ...
        for record in self:
            ...
            text_responses = record.user_input_line_ids.filtered(lambda l: l.value_text).mapped('value_text')
            positive_keywords = [
                'excellent', 'great', 'good', 'satisfied', 'happy', 'pleased',
                'professional', 'efficient', 'helpful', 'timely', 'secure'
            ]
            negative_keywords = [
                'poor', 'bad', 'terrible', 'unsatisfied', 'disappointed', 'slow',
                'unprofessional', 'careless', 'late', 'insecure', 'damaged'
            ]
                ...
                if line.suggested_answer_id and line.suggested_answer_id.value:
                    try:
                        ...
                        if 1 <= rating <= 5:
                            ...
                            if 'overall' in question_title or 'satisfaction' in question_title:
                                return rating
    ...
    def action_assign_followup(self):
        ...
        self.message_post(
            body=_("Follow-up assigned to %s", assigned_user.name if assigned_user else 'Unassigned'),
    ...
    def action_escalate_followup(self):
        ...
        manager = self.env.ref('records_management.group_records_manager').users[:1]
    ...
    def action_create_customer_feedback_record(self):
        ...
        feedback_vals = {
            'name': _("Survey Feedback: %s", self.survey_id.title or 'Unknown Survey'),
    ...
    def _get_followup_assignee(self):
        ...
        if self.records_service_type == 'destruction':
            ...
            return self.env.ref('records_management.group_records_destruction_user').users[:1]
        if self.records_service_type == 'pickup':
            # Assign to field service manager
            return self.env.ref('records_management.group_records_fsm_user').users[:1]
    ...
    def _create_naid_audit_log(self, event_type):
        ...
        """
        if self.env['ir.module.module'].search([('name', '=', 'records_management'), ('state', '=', 'installed')]):
            self.env['naid.audit.log'].create({
                'event_type': event_type,
                'model_name': self._name,
                'record_id': self.id,
                'partner_id': self.records_partner_id.id if self.records_partner_id else False,
                'description': _("Survey response: %s", self.survey_id.title or 'Unknown'),
