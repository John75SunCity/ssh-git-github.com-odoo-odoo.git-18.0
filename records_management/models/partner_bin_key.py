...
class PartnerBinKey(models.Model):
    ...
    active_bin_key_count = fields.Integer(string='Active Bin Key Count', compute='_compute_active_bin_key_count', store=True)
    billable = fields.Boolean(string='Billable', default=False)
    bin_key_history_ids = fields.One2many('bin.key.history', 'partner_bin_key_id', string='Bin Key History Ids')
    ...
    unlock_service_history_ids = fields.One2many('unlock.service.history', 'partner_bin_key_id', string='Unlock Service History Ids')
