...
class PartnerBinKey(models.Model):
    ...
    # binding_model_id = fields.Many2one('binding.model', string='Binding Model Id')  # TODO: Define binding.model or remove
    binding_view_types = fields.Char(string="Binding View Types")
    button_box = fields.Char(string="Button Box")
    category_id = fields.Many2one("res.partner.category", string="Partner Category")
    ...
    def _compute_active_bin_key_count(self):
        ...
        for record in self:
            ...
            record.active_bin_key_count = len(active_keys)

    @api.depends("bin_key_history_ids")  # Using existing field
    def _compute_total_bin_keys_issued(self):
        for record in self:
            # Count total bin keys issued from history
            record.total_bin_keys_issued = len(record.bin_key_history_ids)

    @api.depends("unlock_service_history_ids")  # Using existing field
    def _compute_total_unlock_charges(self):
        for record in self:
            # Sum charges from unlock service history if they have charge amounts
            total = 0
            for service in record.unlock_service_history_ids:
                if hasattr(service, "charge_amount") and service.charge_amount:
                    total += service.charge_amount
            record.total_unlock_charges = total
