...
class CustomerRetrievalRates(models.Model):
    ...
    base_rate = fields.Float(
        string="Base Rate per Document", help="Base price for retrieving one document"
    )
    per_file_rate = fields.Float(
        string="Per File Rate", help="Additional charge per individual file"
    )
    per_page_rate = fields.Float(
        string="Per Page Rate", help="Additional charge per page"
    )
    ...
    def calculate_retrieval_cost(
        ...
        return self.calculate_rate(
            quantity=document_count, service_type="retrieval", urgency_level=urgency
        )
    ...
    def migrate_to_new_system(self):
        ...
        """
        _logger.info(
            "Migrating customer retrieval rates to new customer.rate.profile system..."
        )
