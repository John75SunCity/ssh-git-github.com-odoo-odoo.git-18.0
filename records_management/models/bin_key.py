...
This module provides comprehensive management of physical access keys used    #     # ============================================================================
    ...
    # 'current_holder_id' refers to the internal user (employee) who currently possesses the key.
    current_holder_id = fields.Many2one("res.users",,
    string="Current Holder")cure containers,
...
class BinKey(models.Model):
    ...
    # CORE IDENTIFICATION FIELDS
        # ============================================================================
    name = fields.Char(string="Key Number", required=True, tracking=True,,
    index=True),
    ...
    # KEY SPECIFICATIONS
        # ============================================================================
    key_code = fields.Char(string="Key Code", index=True,,
    tracking=True),
    ...
    default=False),
    key_history_ids = fields.One2many('key.history', 'bin_key_id',,
    string='Key History Ids'),
    ...
    management_record_count = fields.Integer(string='Management Record Count', compute='_compute_management_record_count',,
    store=True),
    management_record_ids = fields.One2many('management.record', 'bin_key_id',,
    string='Management Record Ids'),
    master_keys = fields.Char(string='Master Keys'),
    overdue_returns = fields.Char(string='Overdue Returns'),
    physical_key_id = fields.Many2one('physical.key',,
    string='Physical Key Id'),
