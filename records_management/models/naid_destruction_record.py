...
class NAIDDestructionRecord(models.Model):
    ...
    destruction_date = fields.Datetime(
        string="Destruction Date", default=fields.Datetime.now, tracking=True
    )
    ...
    naid_certificate_id = fields.Many2one(
        "naid.certificate", string="NAID Certificate", tracking=True
    )
    ...
    # CHAIN OF CUSTODY
    # ==========================================
    custody_received_from = fields.Many2one(
        "res.partner", string="Received From", tracking=True
    )
    custody_received_date = fields.Datetime(string="Received Date", tracking=True)
    custody_delivered_by = fields.Many2one(
        "res.users", string="Delivered By", tracking=True
    )
    ...
    witness_ids = fields.One2many(
        "destruction.witness", "destruction_record_id", string="Witnesses"
    )
    witness_count = fields.Integer(
        string="Witness Count", compute="_compute_witness_count", store=True
    )
    ...
    destruction_item_ids = fields.One2many(
        "destruction.item", "destruction_record_id", string="Items Destroyed"
    )

    total_weight = fields.Float(
        string="Total Weight (lbs)", compute="_compute_total_weight", store=True
    )
    ...
    def action_start_destruction(self):
        ...
        self.write({"state": "in_progress", "destruction_date": fields.Datetime.now()})
...
class DestructionWitness(models.Model):
    ...
    destruction_record_id = fields.Many2one(
        "naid.destruction.record", string="Destruction Record", required=True
    )
    partner_id = fields.Many2one(
        "res.partner", string="Witness", required=True, tracking=True
    )
