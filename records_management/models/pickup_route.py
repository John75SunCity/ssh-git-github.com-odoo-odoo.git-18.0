...
class PickupRoute(models.Model):
    ...
    user_id = fields.Many2one('res.users', string='Route Manager', 
    ...
    # ROUTE SCHEDULING
    # ==========================================
    route_date = fields.Date(string='Route Date', required=True, 
    ...
    sequence = fields.Integer(string='Route Sequence', default=10,
                             help='Order of routes for the same date')
    ...
    vehicle_capacity_weight = fields.Float(string='Vehicle Weight Capacity (lbs)',
                                         related='vehicle_id.vehicle_capacity_weight', readonly=True)
    vehicle_capacity_volume = fields.Float(string='Vehicle Volume Capacity (ft³)',
                                         related='vehicle_id.vehicle_capacity_volume', readonly=True)
    ...
    # ROUTE STOPS
    # ==========================================
    route_stop_ids = fields.One2many('pickup.route.stop', 'route_id', 
                                     string='Route Stops')
    stop_count = fields.Integer(string='Number of Stops', 
                               compute='_compute_route_metrics', store=True)
    
    # ==========================================
    # ROUTE METRICS
    # ==========================================
    total_distance = fields.Float(string='Total Distance (miles)', 
                                  compute='_compute_route_metrics', store=True)
    ...
    total_weight = fields.Float(string='Total Weight (lbs)',
                               compute='_compute_load_metrics', store=True)
    total_volume = fields.Float(string='Total Volume (ft³)',
                               compute='_compute_load_metrics', store=True)
    ...
    # ROUTE OPTIMIZATION
    # ==========================================
    optimization_enabled = fields.Boolean(string='Route Optimization Enabled', default=True)
    ...
    def _compute_route_metrics(self):
        ...
        for route in self:
            ...
            route.estimated_duration = (route.stop_count * 0.75) + (route.total_distance / 30.0)
    ...
    @api.depends('total_weight', 'total_volume', 'vehicle_capacity_weight', 'vehicle_capacity_volume')
    def _compute_utilization(self):
        """Compute vehicle utilization percentages"""
        for route in self:
            if route.vehicle_capacity_weight > 0:
                route.weight_utilization = (route.total_weight / route.vehicle_capacity_weight) * 100
            else:
                route.weight_utilization = 0.0
            
            if route.vehicle_capacity_volume > 0:
                route.volume_utilization = (route.total_volume / route.vehicle_capacity_volume) * 100
            else:
    ...
    def action_assign_vehicle(self):
        ...
        self.message_post(body=_('Route assigned to vehicle %s and driver %s') % 
    ...
    def action_complete_route(self):
        ...
        incomplete_stops = self.route_stop_ids.filtered(lambda s: s.status != 'completed')
        if incomplete_stops:
    ...
    def get_efficiency_metrics(self):
        ...
        metrics = {
            ...
            'stops_per_hour': self.stop_count / self.estimated_duration if self.estimated_duration > 0 else 0,
            'miles_per_stop': self.total_distance / self.stop_count if self.stop_count > 0 else 0,
        }
        
        if self.actual_duration > 0:
            metrics.update({
                'actual_stops_per_hour': self.stop_count / self.actual_duration,
                'schedule_variance': ((self.actual_duration - self.estimated_duration) / self.estimated_duration) * 100,
            })
        ...
        for route in self:
            if route.route_stop_ids:
                ...
                if len(sequences) != len(set(sequences)):
                    raise ValidationError(_('Stop sequences must be unique within a route'))
...
class PickupRouteStop(models.Model):
    ...
    ], string='Stop Type', required=True, default='pickup')
    ...
    actual_duration = fields.Float(string='Actual Duration (minutes)', 
                                  compute='_compute_actual_duration', store=True)
    ...
    def _compute_actual_duration(self):
        ...
        for stop in self:
            if stop.actual_arrival_time and stop.actual_departure_time:
                ...
                stop.actual_duration = delta.total_seconds() / 60.0  # Convert to minutes
            else:
        ...
        for stop in self:
            if stop.planned_arrival_time and stop.planned_departure_time:
                if stop.planned_departure_time <= stop.planned_arrival_time:
                    raise ValidationError(_('Planned departure must be after planned arrival'))
