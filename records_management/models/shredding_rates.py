...
class BaseRates(models.Model):
    ...
    ], string='Service Category', required=True, tracking=True)
    ...
    rate_structure = fields.Selection([
        ...
        ('per_page', 'Per Page'),
        ('flat_fee', 'Flat Fee')
    ], string='Rate Structure', required=True, tracking=True)

    base_rate = fields.Float(string='Base Rate ($)', required=True, tracking=True, 
                            help='Base rate amount in dollars')
    minimum_charge = fields.Float(string='Minimum Charge ($)', tracking=True,
    ...
    # Computed fields
    customer_count = fields.Integer(string='Customers Using Base Rate', 
                                   compute='_compute_customer_count')
    negotiated_rate_count = fields.Integer(string='Customers with Negotiated Rates',
    ...
    def _compute_customer_count(self):
        ...
        for record in self:
            ...
            total_customers = self.env['res.partner'].search_count([
                ('customer_rank', '>', 0),
                ('id', 'not in', negotiated_customers)
            ])
    ...
    def _compute_negotiated_rate_count(self):
        ...
        for record in self:
            record.negotiated_rate_count = self.env['customer.rate.profile'].search_count([
                ('service_category', '=', record.service_category),
                ('service_type', '=', record.service_type),
                ('active', '=', True)
            ])
...
class CustomerRateProfile(models.Model):
    ...
    # Base rate reference
    base_rate_id = fields.Many2one('base.rates', string='Base Rate', 
                                  domain="[('service_category', '=', service_category), ('service_type', '=', service_type), ('state', '=', 'active')]",
                                  tracking=True)
    base_rate_amount = fields.Float(string='Base Rate Amount', related='base_rate_id.base_rate', readonly=True)
    ...
    ], string='Adjustment Method', required=True, tracking=True)

    adjustment_value = fields.Float(string='Adjustment Value', required=True, tracking=True,
                                   help='Percentage (without % sign) or dollar amount depending on adjustment method')
    
    # Calculated rate
    negotiated_rate = fields.Float(string='Negotiated Rate ($)', compute='_compute_negotiated_rate', 
                                  store=True, tracking=True)
    rate_difference = fields.Float(string='Difference from Base ($)', compute='_compute_rate_difference',
                                  store=True)
    savings_percentage = fields.Float(string='Savings %', compute='_compute_savings_percentage',
                                     store=True)
    ...
    def _compute_name(self):
        ...
        for record in self:
            if record.partner_id and record.service_category and record.service_type:
                service_name = dict(record._fields['service_type'].selection).get(record.service_type, record.service_type)
    ...
    def _compute_savings_percentage(self):
        ...
        for record in self:
            if record.base_rate_amount:
                record.savings_percentage = ((record.base_rate_amount - record.negotiated_rate) / record.base_rate_amount) * 100
            else:
    ...
    def get_customer_rate(self, partner_id, service_category, service_type):
        ...
        base_rate = self.env['base.rates'].get_active_rate(service_category, service_type)
        if base_rate:
    ...
    def _check_adjustment_value(self):
        ...
        for record in self:
            ...
            elif record.adjustment_method in ['fixed_discount', 'fixed_markup', 'fixed_rate']:
...
class RevenueForecaster(models.TransientModel):
    ...
    global_adjustment_value = fields.Float(string='Adjustment Value', 
                                          help='Percentage (without % sign) or dollar amount')
    ...
    market_growth_rate = fields.Float(string='Expected Market Growth (%)', default=3.0,
    ...
    customer_retention_rate = fields.Float(string='Customer Retention Rate (%)', default=95.0,
    ...
    current_monthly_revenue = fields.Float(string='Current Monthly Revenue ($)', readonly=True)
    projected_monthly_revenue = fields.Float(string='Projected Monthly Revenue ($)', readonly=True)
    revenue_difference = fields.Float(string='Monthly Revenue Change ($)', readonly=True)
    revenue_percentage_change = fields.Float(string='Revenue Change (%)', readonly=True)
    
    annual_revenue_impact = fields.Float(string='Annual Revenue Impact ($)', readonly=True)
    ...
    ], string='Risk Assessment', readonly=True)
    
    # Detailed analysis results
    forecast_line_ids = fields.One2many('revenue.forecast.line', 'forecast_id', 
                                       string='Detailed Forecast Lines', readonly=True)
    ...
    def _compute_name(self):
        ...
        for record in self:
            scenario_name = dict(record._fields['scenario_type'].selection).get(record.scenario_type, 'Forecast')
            period_name = dict(record._fields['forecast_period'].selection).get(record.forecast_period, 'Analysis')
    ...
    def _get_target_customers(self):
        ...
        if self.customer_segment == 'high_volume':
            ...
            domain.extend([('container_count', '>=', 20), ('container_count', '<=', 100)])
            ...
            negotiated_customers = self.env['customer.rate.profile'].search([
                ('state', '=', 'active')
    ...
    def _calculate_customer_monthly_revenue(self, customer):
        ...
        container_count = getattr(customer, 'container_count', 50)  # Default 50 containers
    ...
    def _analyze_category_specific(self):
        ...
        for customer in customers:
            ...
            if self.service_category:
                category_revenue = current_monthly * 0.3  # Assume 30% from this category
                other_revenue = current_monthly * 0.7
                
                adjusted_category_revenue = category_revenue * (1 + self.category_adjustment_value / 100)
    ...
    def _calculate_revenue_changes(self):
        ...
        self.revenue_difference = self.projected_monthly_revenue - self.current_monthly_revenue
        
        if self.current_monthly_revenue:
            self.revenue_percentage_change = (self.revenue_difference / self.current_monthly_revenue) * 100
        else:
    ...
    def _create_forecast_line(self, customer, current_revenue, projected_revenue):
        ...
        self.env['revenue.forecast.line'].create({
            ...
            'revenue_change_percentage': ((projected_revenue - current_revenue) / current_revenue * 100) if current_revenue else 0
        })
...
class RevenueForecastLine(models.TransientModel):
    ...
    forecast_id = fields.Many2one('revenue.forecaster', string='Forecast', required=True, ondelete='cascade')
    ...
    # Customer details
    container_count = fields.Integer(string='Container Count', related='partner_id.container_count', readonly=True)
    customer_segment = fields.Char(string='Customer Segment', compute='_compute_customer_segment')
    risk_level = fields.Selection([
        ('low', 'Low Risk'),
        ('medium', 'Medium Risk'),
        ('high', 'High Risk')
    ], string='Retention Risk', compute='_compute_risk_level')
