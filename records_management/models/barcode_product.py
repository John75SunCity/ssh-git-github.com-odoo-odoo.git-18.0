...
class BarcodeProduct(models.Model):
    ...
    generation_history_ids = fields.One2many('barcode.generation.history', compute='_compute_generation_history_ids', string='Generation History')
    ...
    pricing_tier_ids = fields.One2many('barcode.pricing.tier', compute='_compute_pricing_tier_ids', string='Pricing Tiers')
    ...
    seasonal_pricing_ids = fields.One2many('barcode.seasonal.pricing', compute='_compute_seasonal_pricing_ids', string='Seasonal Pricing')
    ...
    shred_bin_ids = fields.One2many('stock.lot', compute='_compute_shred_bin_ids', string='Shred Bins')
    ...
    storage_box_ids = fields.One2many('stock.lot', compute='_compute_storage_box_ids', string='Storage Boxes')
    ...
    @api.depends()
    def _compute_generation_history_ids(self):
        """Compute generation history for this barcode product"""
        for record in self:
            # Return empty recordset since model doesn't exist
            record.generation_history_ids = self.env['barcode.generation.history'].browse()

    @api.depends()
    def _compute_pricing_tier_ids(self):
        """Compute pricing tiers for this barcode product"""
        for record in self:
            # Return empty recordset since model doesn't exist
            record.pricing_tier_ids = self.env['barcode.pricing.tier'].browse()

    @api.depends()
    def _compute_seasonal_pricing_ids(self):
        """Compute seasonal pricing for this barcode product"""
        for record in self:
            # Return empty recordset since model doesn't exist
            record.seasonal_pricing_ids = self.env['barcode.seasonal.pricing'].browse()

    @api.depends()
    def _compute_shred_bin_ids(self):
        """Compute shred bins for this barcode product"""
        for record in self:
            # Return empty recordset since inverse field doesn't exist
            record.shred_bin_ids = self.env['stock.lot'].browse()

    @api.depends()
    def _compute_storage_box_ids(self):
        """Compute storage boxes for this barcode product"""
        for record in self:
            # Return empty recordset since inverse field doesn't exist
            record.storage_box_ids = self.env['stock.lot'].browse()
