...
class AdvancedBillingProfile(models.Model):
    ...
    @api.depends("billing_line_ids.amount", "billing_frequency", "discount_percentage", "currency_id")
    def _compute_totals(self):
        for profile in self:
            base_amount = sum(line.amount for line in profile.billing_line_ids)
            discounted_amount = base_amount * (1 - (profile.discount_percentage / 100))
            # Convert to monthly equivalent
            if profile.billing_frequency == 'weekly':
                profile.total_monthly_amount = discounted_amount * 4.33  # Approximate weeks per month
