...
class RevenueForecaster(models.Model):
    ...
    category_adjustment_value = fields.Float(
        string="Category Adjustment (%)", help="Percentage adjustment for this category"
    )
    ...
    specific_customer_ids = fields.Many2many(
        "res.partner", string="Target Customers", domain=[("is_company", "=", True)]
    )
    ...
    customer_retention_risk = fields.Float(
        string="Customer Retention Risk (%)", digits=(12, 2), default=5.0
    )
    competitive_response_risk = fields.Float(
        string="Competitive Response Risk (%)", digits=(12, 2), default=10.0
    )
    ...
    # RELATIONSHIPS
    # ==========================================
    rate_change_wizard_ids = fields.One2many(
        "rate.change.confirmation.wizard", "forecast_id", string="Rate Change Wizards"
    )
            ...
            if record.scenario_type == "global_increase":
                if record.global_adjustment_type == "percentage":
                    record.projected_revenue = record.current_revenue * (
                        1 + record.global_adjustment_value / 100
                    )
                else:
                    record.projected_revenue = (
                        record.current_revenue + record.global_adjustment_value
                    )
            elif record.scenario_type == "category_specific":
                # Simplified calculation - could be more sophisticated
                category_impact = (
                    record.current_revenue
                    * 0.3
                    * (record.category_adjustment_value / 100)
                )
    ...
    def _compute_revenue_increase_percentage(self):
        for record in self:
            if record.current_revenue:
                record.revenue_increase_percentage = (
                    record.revenue_increase / record.current_revenue
                ) * 100
            else:
    ...
    def _compute_risk_level(self):
        for record in self:
            total_risk = (
                record.customer_retention_risk + record.competitive_response_risk
            )
    ...
    def action_submit_for_review(self):
        ...
        self.activity_schedule(
            ...
            user_id=(
                self.env.ref("records_management.group_records_manager").users[0].id
                if self.env.ref("records_management.group_records_manager").users
    ...
    def action_implement(self):
        ...
        self.write({"state": "implemented", "implementation_date": fields.Date.today()})
    ...
    def action_create_rate_change_wizard(self):
        ...
        if self.state != "approved":
            raise UserError(
                _("Forecast must be approved before creating rate change wizard")
            )
