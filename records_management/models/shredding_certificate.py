...
class ShreddingCertificate(models.Model):
    ...
    def name_get(self):
        ...
        for record in self:
            ...
            if record.partner_id:
                name = _("%s - %s", name, record.partner_id.name)
            if record.certificate_date:
                name = _("%s (%s)", name, record.certificate_date)
    ...
    def _onchange_destruction_method(self):
        ...
        if self.destruction_method:
            ...
            if base_statement:
                full_statement = "%s This destruction was witnessed and verified according to established chain of custody procedures." % base_statement
