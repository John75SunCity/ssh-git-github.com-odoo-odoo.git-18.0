...
class RecordsChainOfCustody(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get("verification_code"):
                vals["verification_code"] = self.env["ir.sequence"].next_by_code("records.chain.custody") or _("NEW")
        ...
        if "state" in vals:
            for record in self:
                if vals["state"] != record.state:
                    old_state_label = dict(record._fields["state"].selection).get(record.state, record.state)
                    new_state_label = dict(record._fields["state"].selection).get(vals["state"], vals["state"])
                    record.message_post(
        ...
        for record in self:
            if record.custody_from_id and record.custody_to_id:
                if record.custody_from_id == record.custody_to_id:
                    raise ValidationError(_("Cannot transfer custody from and to the same user."))
    ...
    @api.constrains('custody_event', 'priority')
    def _check_priority_event_combination(self):
        """Validate priority and event type combinations"""
        for record in self:
            if record.custody_event == "destroyed" and record.priority not in ["high", "urgent", "critical"]:
                raise ValidationError(_("Destruction events must have high, urgent, or critical priority."))
    ...
    @api.model
    def _search_name_verification_customer(self, name, args=None, operator="ilike", limit=100, name_get_uid=None):
    ...
    @api.model
    def create_automatic_custody_event(self, document_id, event_type, user_from=None, user_to=None, description=None):
