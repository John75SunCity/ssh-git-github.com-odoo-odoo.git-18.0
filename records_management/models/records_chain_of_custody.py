...
class RecordsChainOfCustody(models.Model):
    ...
    document_id = fields.Many2one(
        "records.document", string="Document", tracking=True, ondelete="cascade"
    )
    ...
    work_order_id = fields.Many2one(
        ...
        string="Document Retrieval Work Order",
        tracking=True,
        ondelete="cascade",
    ...
    to_party_id = fields.Many2one(
        "res.partner", string="To Party", tracking=True, help="Party receiving custody"
    )
    custodian_id = fields.Many2one(
        "res.users",
        string="Current Custodian",
        tracking=True,
        help="Current responsible party",
    )
    ...
    from_location_id = fields.Many2one(
        "records.location", string="From Records Location"
    )
    ...
    naid_compliant = fields.Boolean(
        string="NAID AAA Compliant", default=True, tracking=True
    )
    chain_intact = fields.Boolean(
        string="Chain of Custody Intact", default=True, tracking=True
    )
    ...
    previous_log_id = fields.Many2one(
        "records.chain.of.custody.log", string="Previous Log Entry"
    )
    ...
    custody_duration = fields.Float(
        ...
        compute="_compute_custody_duration",
        store=True,
    )
            ...
            if record.naid_compliant and record.verified:
                if record.signature_required:
                    record.compliance_status = (
                        "compliant" if record.signature_obtained else "non_compliant"
                    )
                else:
        ...
        if self.custody_event in ["pickup", "transfer"]:
            ...
            if self.to_party_id:
                ...
                user = self.env["res.users"].search(
                    [("partner_id", "=", self.to_party_id.id)], limit=1
                )
                if user:
    ...
    @api.model
    def create_work_order_custody_log(
        self, work_order_id, custody_event, from_party=None, to_party=None, notes=None
    ):
        """Create custody log entry for document retrieval work order"""
        work_order = self.env["document.retrieval.work.order"].browse(work_order_id)
        ...
        vals = {
            ...
            "location_from": (
                work_order.customer_id.name
                if custody_event == "pickup"
                else "Records Center"
            ),
            "location_to": (
                "Records Center"
                if custody_event == "pickup"
                else work_order.delivery_address
            ),
            ...
            if record.expected_return_date and record.transfer_date:
                if record.expected_return_date <= record.transfer_date:
                    raise ValidationError(
                        _("Expected return date must be after transfer date")
                    )

            if record.actual_return_date and record.transfer_date:
                if record.actual_return_date < record.transfer_date:
                    raise ValidationError(
                        _("Return date cannot be before transfer date")
                    )

    @api.constrains("container_id", "document_id", "work_order_id")
    ...
    def validate_naid_compliance(self):
        ...
        if compliance_issues:
            raise ValidationError(
                _("NAID compliance issues: %s") % ", ".join(compliance_issues)
            )
