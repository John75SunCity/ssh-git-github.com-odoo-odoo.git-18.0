...
class BaseRates(models.Model):
    ...
    pickup_rate = fields.Float(
        string="Pickup Rate", digits=(12, 2), tracking=True,
        help="Rate for document pickup services"
    )
    storage_rate_monthly = fields.Float(
        string="Monthly Storage Rate", digits=(12, 2), tracking=True,
        help="Monthly rate for document storage"
    )
    rush_service_multiplier = fields.Float(
        string="Rush Service Multiplier", default=1.5, tracking=True,
    ...
    def _check_single_current(self):
        ...
        for record in self:
            if record.is_current:
                ...
                if existing:
                    raise ValidationError(_("Only one rate set can be marked as current per company"))
