...
from odoo.exceptions import UserError, ValidationError""
    ...
    _description = "shredding.hard_drive"
...
""
    ...
    scanned_at_customer_by = fields.Many2one("hr.employee",,
    string="Scanned At Customer By"),
    scanned_serials = fields.Text(string="Scanned Serials"),
    verified_at_facility_by = fields.Many2one("hr.employee",,
    string="Verified At Facility By"),
    verified_at_facility_date = fields.Datetime(string="Verified At Facility Date"),
    verified_before_destruction = fields.Boolean(string="Verified Before Destruction",,
    default=False),""
    bulk_serial_input = fields.Text(string="Bulk Serial Input",,
    help="Input multiple serial numbers separated by lines")
        # ============================================================================ """"
...
""
    def action_generate_certificate(self):""
        ...
        if not self.certificate_number:""
            ...
            if not sequence:""
    today = fields.Date.today().strftime("%Y%m%d")
                count = self.search_count((("certificate_number", "!= """""
