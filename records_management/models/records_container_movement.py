...
class RecordsContainerMovement(models.Model):
    ...
    to_location_id = fields.Many2one(
        "records.location", string="To Location", required=True, tracking=True
    )
    ...
    chain_of_custody_ids = fields.One2many(
        "records.chain.of.custody.log", "movement_id", string="Chain of Custody Records"
    )
    ...
    container_barcode = fields.Char(
        related="container_id.name", string="Container Number", readonly=True
    )
    customer_id = fields.Many2one(
        "res.partner",
        related="container_id.customer_id",
        string="Customer",
        readonly=True,
    )
    container_current_location = fields.Many2one(
        "records.location",
        related="container_id.location_id",
        string="Current Container Location",
        readonly=True,
    )
    ...
    movement_status = fields.Char(
        string="Movement Status", compute="_compute_movement_status"
    )
        ...
        for record in self:
            if record.state == "completed":
                ...
                record.movement_status = (
                    f"Scheduled for {record.movement_date.strftime('%Y-%m-%d %H:%M')}"
                )
            else:
    ...
    def action_start_movement(self):
        ...
        self.write({"state": "in_transit", "actual_start_time": fields.Datetime.now()})
    ...
    def _create_custody_record(self):
        ...
        custody_vals = {
            ...
            "from_location_id": (
                self.from_location_id.id if self.from_location_id else False
            ),
        ...
        self.write(
            {
                ...
                "custody_transfer_date": fields.Datetime.now(),
            }
        )
            ...
            if record.actual_start_time and record.movement_date:
                if record.actual_start_time < record.movement_date:
                    raise ValidationError(
                        _("Actual start time cannot be before scheduled movement date")
                    )
