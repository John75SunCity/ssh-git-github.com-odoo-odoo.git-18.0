...
class CustomerNegotiatedRates(models.Model):
    ...
    base_rate_id = fields.Many2one(
        "base.rates", string="Base Rate Set", required=True, tracking=True,
    ...
    def _compute_total_discount(self):
        ...
        for record in self:
            record.total_discount_percent = record.global_discount_percent + record.volume_discount_percent
            ...
            if record.state == 'active':
                ...
                if existing:
                    raise ValidationError(
                        _("Customer %s already has active negotiated rates. "
                          "Please expire existing rates before activating new ones.") % record.partner_id.name
                    )
