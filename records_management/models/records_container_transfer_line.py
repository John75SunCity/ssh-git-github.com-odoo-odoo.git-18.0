...
class RecordsContainerTransferLine(models.Model):
    ...
    from_location_id = fields.Many2one(related='transfer_id.from_location_id', string="From Location", store=True, readonly=True)
    to_location_id = fields.Many2one(related='transfer_id.to_location_id', string="To Location", store=True, readonly=True)
