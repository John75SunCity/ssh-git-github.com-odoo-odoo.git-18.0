...
class ContainerTypeConverter(models.TransientModel):
    ...
    model = fields.Char(string="Model Name", default="records.container.type.converter")
    res_model = fields.Char(
        string="Resource Model", default="records.container.type.converter"
    )
    ...
    efficiency_score = fields.Float(
        string="Efficiency Score", compute="_compute_efficiency_score"
    )
    ...
    @api.depends("container_ids", "new_container_type_code")
    def _compute_name(self):
        for record in self:
            if record.container_ids and record.new_container_type_code:
                type_dict = dict(record._fields["new_container_type_code"].selection)
                type_name = type_dict.get(
                    record.new_container_type_code, record.new_container_type_code
                )
                record.name = f"Convert {len(record.container_ids)} containers to {type_name.split(' - ')[0]}"
            else:
                record.name = "Container Type Converter"

    @api.depends("container_ids")
    def _compute_conversion_count(self):
        for record in self:
            record.conversion_count = len(record.container_ids)

    @api.depends("container_ids", "new_container_type_code")
    def _compute_cost_impact(self):
        for record in self:
            if not record.container_ids or not record.new_container_type_code:
                record.cost_impact = 0.0
                continue

            current_total = sum(
                container.monthly_rate for container in record.container_ids
            )
            rate_mapping = {"01": 0.32, "03": 0.50, "04": 1.25, "06": 2.00}
            new_rate = rate_mapping.get(record.new_container_type_code, 0.32)
            new_total = new_rate * len(record.container_ids)
            record.cost_impact = new_total - current_total

    @api.depends("container_ids", "new_container_type_code", "update_location")
    ...
    def _compute_summary_line(self):
        for wizard in self:
            if not wizard.container_ids or not wizard.new_container_type_code:
                wizard.summary_line = "<p>Please select containers and new type</p>"
                continue

            container_count = len(wizard.container_ids)
            type_mapping = dict(wizard._fields["new_container_type_code"].selection)
            new_type_name = type_mapping.get(wizard.new_container_type_code, "Unknown")

            # Calculate cost impact
            current_total = sum(
                container.monthly_rate for container in wizard.container_ids
            )

            # Calculate new rates
            rate_mapping = {"01": 0.32, "03": 0.50, "04": 1.25, "06": 2.00}
            new_rate = rate_mapping.get(wizard.new_container_type_code, 0.32)
            new_total = new_rate * container_count
            ...
            if wizard.update_location:
                relocation_info = "<br/><small>Containers will be auto-relocated to appropriate locations</small>"

            wizard.summary_line = f"""
                <p><strong>Converting {container_count} containers to {new_type_name}</strong>
    ...
    def default_get(self, fields_list):
        ...
        # Get container IDs from context or active selection
        container_ids = self.env.context.get("default_container_ids")
        if (
            not container_ids
            and self.env.context.get("active_model") == "records.container"
        ):
            active_ids = self.env.context.get("active_ids", [])
            if active_ids:
                container_ids = [(6, 0, active_ids)]

        if container_ids:
            defaults["container_ids"] = container_ids

            # If all containers have same type, show it
            containers = self.env["records.container"].browse(
                [bid for bid in container_ids[0][2]]
            )
            container_types = containers.mapped("container_type_code")
            if len(set(container_types)) == 1:
                defaults["current_type"] = (
                    f"{container_types[0]} ({len(containers)} containers)"
                )

        return defaults

    def action_convert_containers(self):
        """Execute the bulk container type conversion."""
        if not self.container_ids:
            raise ValidationError(_("Please select at least one container to convert"))

        if not self.new_container_type_code:
            raise ValidationError(_("Please select a new container type"))
        ...
        target_location_type = location_type_mapping.get(self.new_container_type_code)
        ...
        if self.update_location and target_location_type:
            ...
            target_location = self.env["records.location"].search(
                [("location_type", "=", target_location_type), ("active", "=", True)],
                limit=1,
            )
        ...
        for container in self.container_ids:
            old_type = container.container_type_code
            old_rate = container.monthly_rate
            old_location = container.location_id

            # Update container type
            container.container_type_code = self.new_container_type_code

            # Update location if requested and target found
            if self.update_location and target_location:
                container.location_id = target_location

            # Log the change
            new_rate = container.monthly_rate
            rate_change = new_rate - old_rate

            log_entry = f"Container {container.barcode}: {old_type} → {self.new_container_type_code}"
            if rate_change != 0:
                log_entry += f" (${old_rate:.2f} → ${new_rate:.2f}/month)"
            if old_location != container.location_id:
                log_entry += (
                    f" Moved: {old_location.name} → {container.location_id.name}"
                )
        ...
        if self.reason:
            log_message = (
                f"Bulk Conversion Reason: {self.reason}\n\nChanges:\n"
                + "\n".join(conversion_log)
            )
        else:
            log_message = "Bulk Container Type Conversion:\n" + "\n".join(
                conversion_log
            )

        # Log activity on first container (as representative)
        if self.container_ids:
            self.container_ids[0].message_post(
                body=log_message, subject="Bulk Container Type Conversion"
            )

        # Show success message
        message = _("Successfully converted %d containers to type %s") % (
            len(self.container_ids),
            dict(self._fields["new_container_type_code"].selection)[
                self.new_container_type_code
            ],
        )
    ...
    def action_preview_changes(self):
        ...
        if not self.container_ids or not self.new_container_type_code:
            raise ValidationError(_("Please select containers and new type first"))

        # Calculate the impact
        rate_mapping = {"01": 0.32, "03": 0.50, "04": 1.25, "06": 2.00}
        new_rate = rate_mapping.get(self.new_container_type_code, 0.32)
        ...
        for container in self.container_ids:
            ...
            preview_data.append(
                {
                    "container": container.name or container.barcode,
                    "current_type": container.container_type_code,
                    "current_rate": old_rate,
                    "new_type": self.new_container_type_code,
                    "new_rate": new_rate,
        ...
        return {
            ...
            "view_mode": "form",
            "target": "new",
            "context": {"preview_data": preview_data, "show_preview": True},
        }
