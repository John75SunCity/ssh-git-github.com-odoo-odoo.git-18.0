...
class FieldLabelPortalController(CustomerPortal):
    ...
    def get_transitory_field_config(self, customer_id=None, department_id=None):
        ...
        try:
            ...
            if department_id:
                config = self._apply_department_overrides(
                    config, department_id, resolved_customer_id
                )
        ...
        try:
            ...
            for field_name, field_value in field_values.items():
                ...
                if field_value and field_config.get("validation_pattern"):
                    if not self._validate_field_pattern(
                        field_value, field_config["validation_pattern"]
                    ):
                        validation_results["errors"][field_name] = _(
...
class FieldLabelAdminController(http.Controller):
    ...
    def apply_label_preset(self, config_id, preset_name):
        ...
        try:
            ...
            # Log the action
            _logger.info(
                "Applied %s preset to configuration %d by user %s",
                preset_name,
                config_id,
                ...
                try:
                    ...
                    if not config_id or not changes:
                        results["errors"].append(
                            _("Invalid update data for config ID: %s", config_id)
                        )
                        results["failed"] += 1
                        continue
                    ...
                    if not config.exists():
                        results["errors"].append(
                            _("Configuration not found: %s", config_id)
                        )
                        results["failed"] += 1
                        continue
    ...
    @http.route(
        ["/records/admin/field-labels/export"],
        type="http",
        auth="user",
        methods=["GET"],
        website=True,
    ...
    def export_label_configuration(self, customer_id=None, format="csv"):
        ...
        try:
            ...
            configs = request.env["field.label.customization"].search(
                domain, limit=EXPORT_LIMIT
            )
    ...
    def _export_xlsx(self, configs, customer_id):
        ...
        for row, config in enumerate(configs, 1):
            worksheet.write(
                row, 0, config.customer_id.name if config.customer_id else ""
            )
            worksheet.write(
                row, 1, config.department_id.name if config.department_id else ""
            )
