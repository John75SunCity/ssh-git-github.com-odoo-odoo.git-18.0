...
class RecordsManagementMonitor(models.Model):
    ...
    ], string='Type', required=True, default='info', tracking=True)
    ...
    severity = fields.Selection([
        ...
        ('critical', 'Critical'),
        ('emergency', 'Emergency')
    ], string='Severity', default='low', tracking=True)
    ...
    @api.model
    def log_error(self, error_msg, traceback_str=None, context=None, severity='medium', 
                  model=None, method=None, record_id=None):
        """
        ...
        try:
            ...
            vals = {
                'name': f"Error: {error_msg[:50]}..." if len(error_msg) > 50 else f"Error: {error_msg}",
                'monitor_type': 'error',
                'severity': severity,
            ...
            if severity in ['high', 'critical', 'emergency']:
                threading.Thread(target=self._send_notifications, args=(monitor_record,)).start()
    ...
    def _send_webhook_notification(self, monitor_record):
        ...
        try:
            ...
            webhook_url = self.env['ir.config_parameter'].sudo().get_param(
                'records_management.monitoring_webhook_url', ''
            )
            ...
            _logger.info(f"Webhook notification sent successfully: {response.status_code}")
    ...
    def _send_email_notification(self, monitor_record):
        ...
        try:
            ...
            notification_email = self.env['ir.config_parameter'].sudo().get_param(
                'records_management.monitoring_email', ''
            )
    ...
    @api.model
    def log_performance(self, operation, execution_time, memory_usage=None, cpu_usage=None, context=None):
    ...
    def health_check(self):
        ...
        try:
            ...
            try:
                ...
                health_data['checks'].append({'name': 'database', 'status': 'error', 'message': str(e)})
            ...
            for model_name in critical_models:
                try:
                    ...
                    health_data['checks'].append({'name': f'model_{model_name}', 'status': 'ok'})
                except Exception as e:
