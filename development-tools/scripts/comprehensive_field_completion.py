...
def get_field_type_from_context(field_name, xml_context=""):
    ...
    if (
        'widget="boolean"' in context_lower
        or 'widget="boolean_toggle"' in context_lower
    ):
    ...
    if any(
        ...
        for word in ["amount", "cost", "price", "rate", "fee", "total"]
    ):
        return "fields.Monetary(string='Amount', currency_field='currency_id')"
    ...
    if (
        ...
        or any(
            word in field_name_lower
            for word in ["enabled", "active", "required", "verified", "confirmed"]
        )
    ):
        return "fields.Boolean(string='Flag', default=False)"
    ...
    if any(
        word in field_name_lower
        for word in ["address", "description", "notes", "comment"]
    ):
        ...
        for field_elem in root.iter("field"):
            ...
            if field_name and field_name not in ["name", "id", "arch", "model"]:
                ...
                if field_elem.getparent() is not None:
                    parent_context = ET.tostring(
                        field_elem.getparent(), encoding="unicode"
                    )
    ...
    try:
        ...
        if field_matches:
            ...
            if class_match:
                ...
                if content[insert_pos:].strip().startswith('"""') or content[
                    insert_pos:
                ].strip().startswith("'''"):
                    docstring_match = re.search(
                        r'(""".*?"""|\'\'\'.*?\'\'\')', content[insert_pos:], re.DOTALL
                    )
                    if docstring_match:
...
def main():
    ...
    for view_file in view_files:
        ...
        print(
            f"  📝 Found {len(missing_fields)} missing fields: {', '.join(missing_fields.keys())}"
        )
