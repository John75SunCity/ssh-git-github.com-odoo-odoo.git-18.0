...
class ComprehensiveValidator:
    def __init__(self, records_management_path, auto_fix=False):
    ...
    def _apply_xml_fixes(self, file_path):
        """Apply automatic fixes to XML files"""
        fixes_made = []
        ...
        try:
            ...
            # Fix 1: Remove unnecessary <data> wrappers in Odoo 18.0
            if '<odoo>' in content and '<data>' in content:
                # Check if all content is wrapped in a single <data> tag
                lines = content.split('\n')
                data_start = -1
                data_end = -1
                odoo_start = -1
                ...
                for i, line in enumerate(lines):
                    if '<odoo>' in line and odoo_start == -1:
                        odoo_start = i
                    elif '<data>' in line and data_start == -1 and i > odoo_start:
                        data_start = i
                    elif '</data>' in line and data_end == -1:
                        data_end = i

                if data_start > -1 and data_end > -1 and data_start < data_end:
                    # Check if there's minimal content between <odoo> and <data>
                    between_content = '\n'.join(lines[odoo_start+1:data_start]).strip()
                    if not between_content or between_content.isspace():
                        # Remove the data wrapper
                        new_lines = (lines[:data_start] +
                                   lines[data_start+1:data_end] +
                                   lines[data_end+1:])
                        content = '\n'.join(new_lines)
                        fixes_made.append("Removed unnecessary <data> wrapper to prevent Odoo 18.0 schema errors")

            # Fix 2: Replace deprecated <list> tags with <tree>
            if '<list' in content:
                old_count = content.count('<list')
                content = re.sub(r'<list(\s[^>]*)?>', r'<tree\1>', content)
                content = content.replace('</list>', '</tree>')
                new_count = content.count('<tree')
                if old_count > 0:
                    fixes_made.append(f"Replaced {old_count} deprecated <list> tags with <tree> tags")

            # Fix 3: Convert deprecated <act_window> tags to proper <record> structure
            act_window_pattern = r'<act_window\s+([^>]+)/>'
            act_windows = re.findall(act_window_pattern, content)

            for act_window_attrs in act_windows:
                # Parse attributes
                attrs = {}
                attr_pattern = r'(\w+)=(["\'])([^"\']*)\2'
                for match in re.finditer(attr_pattern, act_window_attrs):
                    attrs[match.group(1)] = match.group(3)

                if 'id' in attrs:
                    # Create proper record structure
                    record_xml = f'''    <record id="{attrs['id']}" model="ir.actions.act_window">
        <field name="name">{attrs.get('name', attrs['id'])}</field>
        <field name="res_model">{attrs.get('res_model', '')}</field>
        <field name="view_mode">{attrs.get('view_mode', 'tree,form')}</field>'''
                    ...
                    # Replace the act_window with record
                    old_act_window = f'<act_window {act_window_attrs}/>'
                    content = content.replace(old_act_window, record_xml)
                    fixes_made.append(f"Converted deprecated <act_window> to proper <record> structure for {attrs['id']}")

            # Fix 4: Fix common field name corrections
            for old_field, new_field in self.common_corrections.items():
                old_pattern = f'name="{old_field}"'
                new_pattern = f'name="{new_field}"'
                if old_pattern in content:
                    content = content.replace(old_pattern, new_pattern)
                    fixes_made.append(f"Corrected field name: {old_field} → {new_field}")

            # Fix 5: Ensure proper indentation (4 spaces)
            lines = content.split('\n')
            fixed_lines = []
            for line in lines:
                if line.strip():  # Non-empty line
                    ...
                    if leading_space > 0 and leading_space % 4 != 0:
                        # Fix indentation to nearest 4-space boundary
                        proper_indent = (leading_space // 4) * 4
                        if leading_space % 4 >= 2:
                            proper_indent += 4
            ...
            if new_content != original_content:
                if not any("indentation" in fix for fix in fixes_made):
                    fixes_made.append("Fixed XML indentation to use 4-space standard")
    ...
    def validate_single_file(self, file_path):
        """Validate a single XML file with optional auto-fix"""
        ...
        if self.auto_fix:
            ...
            fixed, fixes_made = self._apply_xml_fixes(file_path)
            if fixed:
                print(f"✅ Applied {len(fixes_made)} auto-fixes:")
                for fix in fixes_made:
                    print(f"   - {fix}")
            else:
                print(f"ℹ️  No auto-fixes needed")
    ...
    def validate_all_files(self):
        """Validate all XML files in the views directory with optional auto-fix"""
        ...
        if self.auto_fix:
            print(f"🔧 Auto-fix mode enabled - applying fixes to all files...")
        ...
        for file_path in view_files:
            ...
            if self.auto_fix:
                fixed, fixes_made = self._apply_xml_fixes(file_path)
                if fixed:
                    print(f"✅ {file_name}: Applied {len(fixes_made)} auto-fixes")
    ...
    def print_results(self, file_path=None):
        """Print validation results with auto-fix summary"""
        if file_path:
            # Single file results
            file_name = os.path.basename(file_path)
            ...
            file_fixes = [f for f in self.fixes_applied if f['file'] == file_name]
            ...
            # Show fixes applied
            if file_fixes:
                print(f"🔧 AUTO-FIXES APPLIED ({len(file_fixes[0]['fixes'])}):")
                for fix in file_fixes[0]['fixes']:
                    print(f"   ✅ {fix}")
                print(f"   💾 Backup saved: {os.path.basename(file_fixes[0]['backup'])}")
                print()
            ...
            print(f"\n📊 VALIDATION COMPLETE:")
            if self.auto_fix:
                print(f"   - Files auto-fixed: {total_fixes}")
                if total_fixes > 0:
                    total_individual_fixes = sum(len(f['fixes']) for f in self.fixes_applied)
                    print(f"   - Total fixes applied: {total_individual_fixes}")
            ...
            if self.auto_fix and self.fixes_applied:
                print(f"\n🔧 AUTO-FIX SUMMARY:")
                for fix_info in self.fixes_applied[:10]:  # Show first 10
                    print(f"   ✅ {fix_info['file']}: {len(fix_info['fixes'])} fixes")
                if len(self.fixes_applied) > 10:
                    print(f"   ... and {len(self.fixes_applied) - 10} more files")
