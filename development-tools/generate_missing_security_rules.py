...
def generate_missing_security_rules():
    ...
    missing_security_models = [
        'customer.rate.profile', 'shredding.picklist.item', 'stock.move.sms.validation',
        'stock.report_reception_report_label', 'naid.performance.history', 'load',
        'temp.inventory', 'prod.ext', 'installer', 'records_management.bale',
        'records.billing.service', 'naid.chain.custody', 'records.billing.config',
        'barcode.models.enhanced', 'bin.unlock.service', 'records.container.transfer',
        'records.deletion.request.enhanced', 'records.department.billing.enhanced',
        'records.chain.of.custody', 'barcode.product', 'records.advanced.billing.period',
        'records.container.movement', 'transitory.field.config', 'records.policy.version',
        'records.storage.department.user', 'res.partner.key.restriction', 'records.container',
        'records.approval.workflow', 'container.contents', 'unlock.service.history',
        'bin.key.history', 'paper.load.shipment', 'records.approval.step', 'paper.bale.recycling',
        'destruction.item', 'shredding.hard_drive', 'records.location.inspection',
        'naid.custody.event', 'records.department.billing.contact', 'visitor',
        'records.access.log'
    ]
    ...
    records_management_path = "/workspaces/ssh-git-github.com-odoo-odoo.git-18.0/records_management"
    access_csv_path = os.path.join(records_management_path, "security", "ir.model.access.csv")
    ...
    try:
        ...
        existing_rules = [['id', 'name', 'model_id:id', 'group_id:id', 'perm_read', 'perm_write', 'perm_create', 'perm_unlink']]
    ...
    for model_name in missing_security_models:
        ...
        user_rule = [
            ...
            "1", "1", "1", "0"  # read, write, create, no delete for users
        ]
        ...
        manager_rule = [
            ...
            "1", "1", "1", "1"  # full access for managers
        ]
