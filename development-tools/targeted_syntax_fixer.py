...
class TargetedSyntaxFixer:
            ...
            if not in_string:
                for quote in ['"""', "'''", '"', "'"]:
                    if quote in stripped:
                        if stripped.count(quote) % 2 == 1:  # Odd number means opening string
                            in_string = True
                            string_delimiter = quote
                            break
            else:
                if string_delimiter in stripped:
                    if stripped.count(string_delimiter) % 2 == 1:  # Odd number means closing string
                        in_string = False
                        string_delimiter = None
    ...
    def fix_chain_of_custody_specific(self, file_path):
        ...
        content = re.sub(r'custody_transfer_ids\s*=\s*fields\.One2many\([^)]*\)', 
