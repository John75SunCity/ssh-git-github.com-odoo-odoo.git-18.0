...
class BracketBalanceFixer:
        ...
        for i, line in enumerate(lines):
            ...
            # Pattern 2: Missing opening parenthesis
                ...
                if 'fields.' in fixed_line and '(' in fixed_line:
                    ...
                    if first_paren != -1:
                        fixed_line = fixed_line[:first_paren+1] + '(' + fixed_line[first_paren+1:]
    ...
    def fix_specific_patterns(self, content):
        ...
        content = re.sub(r'= fields\.([A-Za-z0-9_]+)\([^)]*\)\)', 
            ...
            if os.path.exists(root):
                for file in os.listdir(root):
                    if file.endswith('.py'):
                        ...
                        is_valid, error = self.check_syntax(file_path)
                        if not is_valid and ('unmatched' in error or 'bracket' in error or 'paren' in error):
