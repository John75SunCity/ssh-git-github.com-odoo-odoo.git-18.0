...
class FsmTaskServiceLine(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", related="task_id.company_id", store=True
    )
    ...
    approval_method = fields.Selection(
        [("signature", "Signature"), ("email", "Email"), ("verbal", "Verbal")],
    ...
    def action_reject_service(self):
        ...
        self.task_id.message_post(
            body=_("Additional service rejected: %s") % self.service_name
        )
        return True
        ...
        if "state" in vals:
            for record in self:
                ...
                if old_state_label != new_state_label:
                    record.message_post(
                        body=_("Service '%s' status changed from %s to %s")
                        % (record.service_name, old_state_label, new_state_label)
                    )
