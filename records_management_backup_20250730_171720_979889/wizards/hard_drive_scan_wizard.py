...
class HardDriveScanWizard(models.TransientModel):
    ...
    serial_number = fields.Char(
        string="Serial Number / Barcode", help="Scan or enter serial number"
    )
    scanned_serials = fields.Text(
        string="Scanned Serial Numbers",
        readonly=True,
    ...
    bulk_serial_input = fields.Text(
        string="Bulk Serial Input", help="Paste multiple serial numbers (one per line)"
    )
    ...
    scanned_at_customer_by = fields.Many2one(
        "res.users", string="Scanned by (Customer)"
    )
    scanned_at_customer_date = fields.Datetime(string="Customer Scan Date")

    # Facility verification tracking
    verified_at_facility_by = fields.Many2one(
        "res.users", string="Verified by (Facility)"
    )
    ...
    included_in_certificate = fields.Boolean(
        string="Include in Certificate", default=True
    )
    ...
    target = fields.Char(
        string="Target", default="new", help="Window target for wizard display"
    )
    view_mode = fields.Char(
        string="View Mode", default="form", help="View mode for wizard display"
    )
        ...
        if existing:
            if self.scan_location == "customer":
                if existing.scanned_at_customer:
                    raise UserError(
                        _("Serial number %s already scanned at customer location.")
                        % self.serial_number
                    )
                else:
                ...
                if existing.verified_before_destruction:
                    raise UserError(
                        _("Serial number %s already verified at facility.")
                        % self.serial_number
                    )
                else:
            ...
            self.env["shredding.hard_drive"].create_from_scan(
                self.service_id.id, self.serial_number, self.scan_location
            )
