...
class RecordsLocation(models.Model):
    ...
    security_level = fields.Selection(
        [
            ("low", "Low Security"),    action_location_report = fields.Char(string='Action Location Report')
...
activity_ids = fields.One2many('mail.activity', 'res_id', string='Activities', auto_join=True)
    ...
    box_ids = fields.One2many('records.box', 'records_location_id', string='Box Ids')
    ...
    high_availability = fields.Char(string='High Availability')
    inactive = fields.Boolean(string='Inactive', default=False)
    many_open_spaces = fields.Char(string='Many Open Spaces')
