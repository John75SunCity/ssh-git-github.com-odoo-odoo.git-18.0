...
class RecordsContainer(models.Model):
    ...
    source_transitory_id = fields.Many2one(
        "transitory.items",
        string="Source Transitory Item",
        readonly=True,
        tracking=True,
    ...
    department_id = fields.Many2one(
        ...
        string="Department",
        tracking=True,
        help="Department responsible for this container",
    )
    location_id = fields.Many2one(
        "records.location", string="Storage Location", tracking=True
    )
    parent_container_id = fields.Many2one(
        "records.container", string="Parent Container", tracking=True
    )
    ...
    confidential_documents = fields.Boolean(
        string="Contains Confidential Documents",
        compute="_compute_confidential_status",
        store=True,
    )
    ...
    service_request_ids = fields.One2many(
        "portal.request", "container_id", string="Service Requests"
    )
    ...
    received_date = fields.Date(
        string="Received Date", default=fields.Date.today, required=True, tracking=True
    )
    ...
    last_accessed_date = fields.Char(
        string="Last Accessed Date", help="Last access date"
    )
    ...
    # ==========================================
    child_container_ids = fields.One2many(
        "records.container", "parent_container_id", string="Child Containers"
    )
...
activity_ids = fields.One2many('mail.activity', 'res_id', string='Activities', auto_join=True)
    ...
    from_location_id = fields.Many2one('from.location', string='From Location Id')
    ...
    group_by_stored_date = fields.Date(string='Group By Stored Date')
...
message_ids = fields.One2many('mail.message', 'res_id', string='Messages', auto_join=True)
    moved_by_id = fields.Many2one('moved.by', string='Moved By Id')
    movement_date = fields.Date(string='Movement Date')
    movement_type = fields.Selection([], string='Movement Type')  # TODO: Define selection options
    ...
    requests = fields.Char(string='Requests')
    ...
    to_location_id = fields.Many2one('to.location', string='To Location Id')
    ...
    def _compute_confidential_status(self):
        ...
        for record in self:
            record.confidential_documents = any(
                record.document_ids.mapped("confidential")
            )
    ...
    def _compute_container_type_display(self):
        ...
        for record in self:
            if record.container_type:
                ...
                display = dict(record._fields["container_type"].selection).get(
                    record.container_type, record.container_type
                )
    ...
    def action_index_container(self):
        ...
        self.write({"state": "indexed", "indexed_date": fields.Date.today()})
