...
class CustomerBillingProfile(models.Model):
    ...
    storage_advance_months = fields.Integer(string='Storage Advance Months', 
    ...
    # BILLING SCHEDULE
    # ==========================================
    billing_day = fields.Integer(string='Billing Day of Month', 
                                default=1, required=True, tracking=True)
    invoice_due_days = fields.Integer(string='Invoice Due Days', 
                                    default=30, required=True, tracking=True)
    ...
    auto_send_statements = fields.Boolean(string='Auto Send Statements', 
                                        default=False, tracking=True)
    ...
    # RELATED RECORDS
    # ==========================================
    billing_contact_ids = fields.One2many('records.billing.contact', 'billing_profile_id',
    ...
    def _cron_generate_scheduled_invoices(self):
        ...
        for profile in active_profiles:
            try:
                ...
                _logger.error(f'Error generating invoices for profile {profile.name}: {e}')
    ...
    def _check_prepaid_discount_percent(self):
        ...
        for record in self:
            if record.prepaid_discount_percent < 0 or record.prepaid_discount_percent > 100:
                raise ValidationError(_('Prepaid discount percent must be between 0 and 100'))
...
class BillingContact(models.Model):
    ...
    # ==========================================
    billing_profile_id = fields.Many2one('records.customer.billing.profile', 
                                        string='Billing Profile', required=True, ondelete='cascade')
