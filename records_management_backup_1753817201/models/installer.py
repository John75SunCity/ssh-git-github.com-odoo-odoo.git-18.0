...
class RecordsManagementInstaller(models.TransientModel):
    ...
    # ==========================================
    name = fields.Char(
        string="Installation Name", default="Records Management Setup", required=True
    )
    ...
    configure_sequences = fields.Boolean(
        string="Configure Sequences",
        default=True,
        help="Automatically configure number sequences",
    )
    setup_default_locations = fields.Boolean(
        string="Setup Default Locations",
        default=True,
        help="Create default storage locations",
    )
    install_barcode_scanner = fields.Boolean(
    ...
    # MODULE DEPENDENCIES
    # ==========================================
    stock_installed = fields.Boolean(
        string="Stock Module", compute="_compute_module_status", store=True
    )
    account_installed = fields.Boolean(
        string="Accounting Module", compute="_compute_module_status", store=True
    )
    sale_installed = fields.Boolean(
        string="Sales Module", compute="_compute_module_status", store=True
    )
    pos_installed = fields.Boolean(
        string="Point of Sale Module", compute="_compute_module_status", store=True
    )
    ...
    installation_date = fields.Datetime(
        string="Installation Date", default=fields.Datetime.now
    )
    ...
    setup_portal_access = fields.Boolean(
        string="Setup Customer Portal",
        default=True,
    ...
    def install_required_modules(self) -> Dict:
        ...
        self.write(
            {"progress_percentage": 25.0, "current_step": "Dependencies installed"}
        )
    ...
    def action_start_installation(self):
        ...
        try:
                ...
                self.write(
                    {
                        ...
                        "current_step": "Sequences configured",
                    }
                )
                ...
                self.write(
                    {
                        ...
                        "current_step": "Default locations created",
                    }
                )
            ...
            if self.install_demo_data:
                ...
                self.write(
                    {"progress_percentage": 80.0, "current_step": "Demo data installed"}
                )
            ...
            return {
                ...
                "params": {
                    ...
                    "message": _(
                        "Records Management has been successfully installed and configured."
                    ),
                    "type": "success",
                    "sticky": False,
                },
    ...
    def _setup_default_locations(self):
        ...
        for name, location_type in default_locations:
            existing = self.env["records.location"].search(
                [("name", "=", name)], limit=1
            )
            if not existing:
    ...
    def _final_configuration(self):
        ...
        if self.default_retention_days:
            existing_policy = self.env["records.retention.policy"].search(
                [("name", "=", "Default Retention Policy")], limit=1
            )
