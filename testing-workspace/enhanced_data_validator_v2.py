#!/usr/bin/env python3
"""
Enhanced Data File Validator with Core Models
============================================
...
def get_model_fields(model_name):
    ...
    try:
        ...
        # For files with multiple models, extract fields for specific model
        if model_name in [
            "records.billing.contact",
            "records.customer.billing.profile",
        ]:
            # Find the specific model class section
            if model_name == "records.billing.contact":
                # Look for the class after records.customer.billing.profile
                pattern = r'class.*?_name\s*=\s*[\'"]records\.billing\.contact[\'"].*?(?=class|\Z)'
            else:
                # Look for the first class with records.customer.billing.profile
                pattern = r'class.*?_name\s*=\s*[\'"]records\.customer\.billing\.profile[\'"].*?(?=class.*?_name\s*=\s*[\'"]records\.billing\.contact[\'"])'

            match = re.search(pattern, content, re.DOTALL)
            if match:
                model_content = match.group(0)
                field_pattern = r"(\w+)\s*=\s*fields\."
                fields = set(re.findall(field_pattern, model_content))
                return fields

        # For single model files, extract all fields
        field_pattern = r"(\w+)\s*=\s*fields\."
