...
class FinalBracketRepairer:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if close_parens > open_parens:
                ...
                for _ in range(excess):
                    ...
                    if last_close != -1:
                        fixed_line = (
                            fixed_line[:last_close] + fixed_line[last_close + 1 :]
                        )
    ...
    def fix_field_definitions(self, content):
        ...
        content = re.sub(
            r"= fields\.(\w+)\([^)]*\)\)", r'= fields.\1(string="Field")', content
        )
    ...
    def process_bracket_errors(self):
        ...
        print(
            f"❌ Files still needing attention: {len(problem_files) - len(self.fixed_files)}"
        )
