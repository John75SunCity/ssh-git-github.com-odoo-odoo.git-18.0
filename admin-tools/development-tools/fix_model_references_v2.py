    ...
    for file_path in python_files:
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                # Find _name = 'model.name' patterns - be more specific
                matches = re.findall(r"_name\s*=\s*['\"]([a-z][a-z0-9_.]*)['\"]", content)
                for match in matches:
                    # Only include if it looks like a real model name (has dots, reasonable length)
                    if '.' in match and len(match) > 3 and not match.startswith(('def ', 'class ')):
                        models.add(match)
...
def main():
    ...
    for xml_file in xml_files:
        ...
        # Only check files that might have model references
        if any(keyword in xml_file for keyword in ['data/', 'views/', 'report/']):
