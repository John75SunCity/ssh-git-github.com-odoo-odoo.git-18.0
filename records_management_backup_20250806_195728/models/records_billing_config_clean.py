...
class RecordsBillingConfig(models.Model):
    ...
    minimum_charge = fields.Monetary(
        ...
        currency_field="currency_id",
        help="Minimum charge per billing cycle",
    )
    ...
    last_modified_date = fields.Datetime(
        string="Last Modified", default=fields.Datetime.now, readonly=True
    )
    ...
    def calculate_storage_cost(self, box_count, months=1):
        ...
        self.ensure_one()
        base_cost = self.default_storage_rate * box_count * months

        if base_cost < self.minimum_charge and months == 1:
    ...
    def calculate_prepaid_discount(self, amount, months):
        ...
        self.ensure_one()
        if not self.prepaid_enabled or months < self.prepaid_minimum_months:
            return 0.0
...
class RecordsBillingConfigLine(models.Model):
    ...
    def name_get(self):
        result = []
        for record in self:
            name = f"{record.name} ({record.service_type.title()}) - {record.rate} {record.currency_id.symbol}"
